...
def bereken_worp(request):
    ...
    if request.method == "POST":
        ...
        def beginsnelheid_km_per_h() -> float | None:
            nonlocal error
            try:
        ...
        def beginhoek_graden() -> float | None:
            nonlocal error
            try:
        ...
        def print_berekende_worp(beginsnelheid_km_per_h: float, beginhoek_graden: float, vliegtijd_seconden: float, vliegafstand_meter: float):
            nonlocal result, aantal_decimalen
            result += f"De beginsnelheid was {round(beginsnelheid_km_per_h, aantal_decimalen)} km/h.<br>"
            result += f"De beginhoek was {round(beginhoek_graden, aantal_decimalen)} graden.<br>"
        ...
        beginsnelheid_km_per_h = beginsnelheid_km_per_h()
        beginhoek_graden = beginhoek_graden()
        if beginsnelheid_km_per_h and beginhoek_graden:
            ...
            print_berekende_worp(beginsnelheid_km_per_h, beginhoek_graden, vliegtijd_seconden, vliegafstand_meter)

            WorpData.objects.create(
                beginsnelheid_km_per_h = round(beginsnelheid_km_per_h, aantal_decimalen),
                beginhoek_graden = round(beginhoek_graden, aantal_decimalen),
